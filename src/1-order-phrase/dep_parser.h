...
public:
    ...
    virtual void GetBestK(const vector<SeqNode> &seq, vector<DepTree> *k_best_list, int K);
    virtual void Fit(const vector<vector<SeqNode> > &train_data,
    ...
    virtual void Test(const vector<vector<SeqNode> > &test_data,
                const vector<DepTree> &test_gold_trees,
                bool dump,
                double* uas,
                double* las);
    virtual void LoadParam();
    virtual void LoadParam(map<string, int> *word_dict,
    ...
    virtual void SaveParam(const vector<string> &id_to_word,
...
protected:
    ...
    virtual void Decode(int sen_len, 
                ...
                Cache* cache, 
                DepTree* tree);
    virtual double GetArcScore(int pos1, int pos2, int arc_dir, 
                       const DepTree &gold_tree, 
                       double penalty, 
                       bool train,
                       Cache* cache, 
    ...
    virtual void PreCompute(int sen_len, const MatrixXd &sen_word_vec, const MatrixXd &sen_pos_vec, Cache* cache);
    ...
    virtual double BackProp(const DepTree &best_tree,
                    const DepTree &gold_tree,
                    const Cache &cache,
                    const vector<SeqNode> &data);
    virtual void BackPropArc(int start, int end, int right, int sign,
                     const Cache &cache,
                     const vector<SeqNode> &data);
    ...
    bool GradientCheck(const DepTree &best_tree,
                       ...
                       MatrixXd* mat);
    virtual void UpdateParam(double alpha, double l2_reg, int batch_size);
